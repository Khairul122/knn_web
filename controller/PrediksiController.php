...
class PrediksiController {
    ...
    public function index() {
        ...
        try {
            ...
            if ($statistics['total_prediksi'] > 0) {
                try {
                    $overfittingCheck = $this->model->detectOverfitting($statistics['last_k_value']);
    ...
    private function findOptimalK() {
        try {
            ...
            $result = $this->model->findOptimalK($maxK);
            
            return [
                'success' => $result['success'],
                'data' => $result,
                'message' => $result['success'] ? 
                    "K optimal ditemukan: K={$result['optimal_k']} dengan accuracy {$result['best_accuracy']}%" :
                    $result['message']
            ];
